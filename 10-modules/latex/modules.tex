...
\begin{tikzpicture}[
    ...
    ->/.style={line join=round, decorate, decoration={zigzag, segment length=4, amplitude=.9, post=lineto, post length=2pt}}
    ]
    \node[kind](knd) at (0, 2){$\knd$};
    \node[type](utyp) at (-1, 1){$\typ$};
    \node[type](typ) at (1, 1){$\typ$};
    \node[term](uexp) at (-1, 0){$\uexp$};
    \node[term](exp) at (1, 0){$\exp$};
    \node[signature](usig) at (3, 2){$\sig$};
    \node[signature](sig) at (5, 2){$\sig$};
    \node[module](umod) at (3, 1){$\mod$};
